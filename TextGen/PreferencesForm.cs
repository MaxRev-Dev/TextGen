...
namespace TextGen
{
    public partial class PreferencesForm : Form
    {
        ...
        {
            using (var x = new OpenFileDialog { FileName = "", Filter = @"HTML файл|*.html", Title = label1.Text })
            {
                x.ShowDialog();
        ...
        {
            using (var x = new OpenFileDialog {Filter = @"HTML файл|*.html", Title = label2.Text})
            {
                x.ShowDialog();
                if (x.FileName != "")
                {
        ...
        {
            using (var x = new OpenFileDialog {Filter = @"Tекстовий файл|*.txt"})
            {
                x.ShowDialog();
                x.Title = label3.Text;
                if (x.FileName != "")
